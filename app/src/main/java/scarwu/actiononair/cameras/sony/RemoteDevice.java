...
import scarwu.actiononair.libs.XmlParser;
...
public final class RemoteDevice {
    ...
    private static final String TAG = "AoA-" + RemoteDevice.class.getSimpleName();
    ...
    private RemoteDevice() {
        mApiServices = new ArrayList<RemoteDevice.ApiService>();
    }
     ...
     */
    public static RemoteDevice fetch(String ddUrl) {
        if (ddUrl == null) {
        ...
        XmlParser rootElement = XmlParser.parse(ddXml);
        ...
        if ("root".equals(rootElement.getTagName())) {
            ...
            // "device"
            XmlParser deviceElement = rootElement.findChild("device");
            ...
            // "iconList"
            XmlParser iconListElement = deviceElement.findChild("iconList");
            List<XmlParser> iconElements = iconListElement.findChildren("icon");
            for (XmlParser iconElement : iconElements) {
